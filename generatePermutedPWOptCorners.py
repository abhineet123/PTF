...
if __name__ == '__main__':
    ...
    tx_vec, ty_vec, theta_vec, scale_vec, a_vec, b_vec, v1_vec, v2_vec, tx2_vec, ty2_vec = getGridVectors(vec_min, vec_max, vec_res)
    ...
    if upd_template:
        tracker_type += 'u'
    tracker_type += str(selective_opt) + '_' + opt_type + '_' + inc_type
    ...
    for frame_id in xrange(start_id, end_id):
        ...
        if upd_template:
            ...
            init_pixel_vals = post_proc_func(init_pixel_vals)
        if show_img:
            drawRegion(curr_img, curr_corners, proj_col, 1)
