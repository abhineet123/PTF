...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def loadFile(self, filePath=None):
        ...
        if self.frames_reader is None:
            if os.path.isdir(filePath):
                frames_reader = DirectoryReader(filePath, save_as_bin=0)
                if not frames_reader.setROI(self.roi):
                    self.roi = None
                if frames_reader.num_frames == 0:
                    QMessageBox.critical(self, "Error", "No frames detected! Please choose different path.",
                                         QMessageBox.Ok)
                    del frames_reader
                    return
                ...
                frames_reader = VideoReader(filePath, save_as_bin=0)
                if frames_reader.num_frames == 0:
                    QMessageBox.critical(self, "Error", "No frames detected! Please choose different path.",
                                         QMessageBox.Ok)
                    del frames_reader
                    return
            self.mImgList = frames_reader.get_frames_list()
    ...
    def augment(self, request, mask=None, use_prev=0, save_seq=1):
        ...
        # col = 1
        col = 255
        mask_img, _ = Shape.contourPtsToMask(_contour_pts, patch_img, col=(col, col, col))
