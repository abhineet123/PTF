...
for i=1:surf_count
    ...
    fprintf('grid_type: %5s x_id: %2d, y_id: %2d, x_label: %5s y_label: %5s dist_fname: %s\n', grid_type,...
        x_id, y_id, x_label, y_label, dist_fname);
    ...
    plot_title_templ{i}=sprintf('%s_%s_%s_%s_%s%d', inc_type, grid_type, seq_name,...
        appearance_model, filter_type, kernel_size);
end
...
while ~end_exec
    ...
    if frame_id~=last_frame_id
        ...
        for i=1:surf_count
            ...
            curr_grid_pos=grid_start_pos(i) + dist_grid_sizes(i)*(curr_frame_id-file_start_id);
            ...
            if tracking_mode
                ...
                for j=1:n_iters(curr_frame_id)                      
                    z_val=interp2(surf_x, surf_y, surf_z, x_vals(curr_frame_id, j), y_vals(curr_frame_id, j));
                    if isnan(z_val)
                        z_val=0;
                    end
                ...
                fprintf('n_iters:\t%d\t', n_iters(curr_frame_id));
                hold off;
            end   
