#include "xvSSDSE2Py.h"

/* ==== Set up the methods table ====================== */
static PyMethodDef xvSSDSE2PyMethods[] = {
...
PyMODINIT_FUNC initxvSSDSE2Py()  {
    (void) Py_InitModule("xvSSDSE2Py", xvSSDSE2PyMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "O!ddddiii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
        ...
        printf("\n----xvSSDSE2Py: initialize: Input arguments could not be parsed----\n\n");
        return NULL;
    }

    if (in_img_py == NULL) {
        printf("\n----xvSSDSE2Py::update: in_img_py is NULL----\n\n");
        return NULL;
    }
	...
	if (new_tracker->direct_capture) {
		...
		if(vid) {
			...
			if (fp==NULL) {
				fprintf(stdout, "xvSSDSE2Py::initialize: File xvInput.bin could not be opened for reading\n");
				return NULL;
			}
		...
		if(!vid) {
			printf("xvSSDSE2Py::initialize: vid was not initialized successfully\n");
			return NULL;
		} else {
			printf("xvSSDSE2Py::initialize: vid=%d\n",vid);
		...
		printf("xvSSDSE2Py::initialize: img_width=%d img_height=%d\n", img_width, img_height);
...
static PyObject* update(PyObject* self, PyObject* args) {
    ...
    if (current_tracker->direct_capture) {
        //printf("\n----xvSSDTrans: update: Using direct capture----\n\n");
        ...
        if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
            printf("\n----xvSSDSE2Py::update: Input arguments could not be parsed----\n\n");
            return NULL;
        }
