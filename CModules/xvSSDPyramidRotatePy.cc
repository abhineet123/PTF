#include "xvSSDPyramidRotatePy.h"
...
PyMODINIT_FUNC initxvSSDPyramidRotatePy()  {
    (void) Py_InitModule("xvSSDPyramidRotatePy", xvInputMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "O!ddddiidii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
		...
		&new_tracker->direct_capture, &new_tracker->show_xv_window)) {
			printf("\n----xvSSDPyramidalRotatePy::initialize: Input arguments could not be parsed----\n\n");
			return NULL;
	}

	if (in_img_py == NULL) {
		printf("\n----xvSSDPyramidalRotatePy::update: in_img_py is NULL----\n\n");
		return NULL;
	}

	printf("Initializing Pyramidal Rotate SSD Tracker with:\n\t");
	printf("size_x=%f\n\t",size_x);
	printf("size_y=%f\n\t",size_y);
	...
	if (new_tracker->direct_capture) {
		...
		if(vid) {
			...
			if (fp==NULL) {
				fprintf(stdout, "xvSSDPyramidalRotatePy::initialize: File xvInput.bin could not be opened for reading\n");
				return NULL;
			}
		...
		if(!vid) {
			printf("xvSSDPyramidalRotatePy::initialize: vid was not initialized successfully\n");
			return NULL;
		} else {
		...
		printf("xvSSDPyramidalRotatePy::initialize: img_width=%d img_height=%d\n", img_width, img_height);
...
static PyObject* update(PyObject* self, PyObject* args) {
    ...
    if (current_tracker->direct_capture) {
        //printf("\n----xvSSDPyramidalRotatePy: update: Using direct capture----\n\n");
        ...
        if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
            printf("\n----xvSSDPyramidalRotatePy::update: Input arguments could not be parsed----\n\n");
            return NULL;
        }
