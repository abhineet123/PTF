#include "xvSSDPyramidSE2Py.h"

/* ==== Set up the methods table ====================== */
static PyMethodDef xvSSDPyramidSE2PyMethods[] = {
...
PyMODINIT_FUNC initxvSSDPyramidSE2Py()  {
    (void) Py_InitModule("xvSSDPyramidSE2Py", xvSSDPyramidSE2PyMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
    ...
    if (!PyArg_ParseTuple(args, "O!ddddiidii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
        ...
        printf("\n----xvSSDPyramidalSE2::initialize: Input arguments could not be parsed----\n\n");
        return NULL;
    }

    if (in_img_py == NULL) {
        printf("\n----xvSSDPyramidalSE2::update: in_img_py is NULL----\n\n");
        return NULL;
    }

    printf("Initializing Pyramidal SE2 SSD Tracker with:\n\t");
    printf("size_x=%f\n\t",size_x);
    printf("size_y=%f\n\t",size_y);
    ...
    if (new_tracker->direct_capture) {
        ...
        if(vid) {
            ...
            if (fp==NULL) {
                fprintf(stdout, "xvSSDPyramidalSE2::initialize: File xvInput.bin could not be opened for reading\n");
                return NULL;
            }
        ...
        if(!vid) {
            printf("xvSSDPyramidalSE2::initialize: vid was not initialized successfully\n");
            return NULL;
        } else {
        ...
        printf("xvSSDPyramidalSE2::initialize: img_width=%d img_height=%d\n", img_width, img_height);
	...
	new_tracker->ssd->initState(state);
...
static PyObject* update(PyObject* self, PyObject* args) {
    ...
    if (current_tracker->direct_capture) {
        //printf("\n----xvSSDPyramidalSE2: update: Using direct capture----\n\n");
        ...
        if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
            printf("\n----xvSSDPyramidalSE2::update: Input arguments could not be parsed----\n\n");
            return NULL;
        }
