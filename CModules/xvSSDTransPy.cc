...
PyMODINIT_FUNC initxvSSDTransPy()  {
	(void) Py_InitModule("xvSSDTransPy", xvInputMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "O!ddddiii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
		&size_x, &size_y, &new_tracker->steps_per_frame,
		&new_tracker->direct_capture, &new_tracker->show_xv_window)) {
			printf("\n----xvSSDTransPy::initialize: Input arguments could not be parsed----\n\n");
			return NULL;
	}

	if (in_img_py == NULL) {
		printf("\n----xvSSDTransPy::update: in_img_py is NULL----\n\n");
		return NULL;
	}
	...
	if (new_tracker->direct_capture) {
		...
		if(vid) {
			...
			if (fp==NULL) {
				fprintf(stdout, "xvSSDTransPy::initialize: File xvInput.bin could not be opened for reading\n");
				return NULL;
			}
		...
		if(!vid) {
			printf("xvSSDTransPy::initialize: vid was not initialized successfully\n");
			return NULL;
		} else {
		...
		printf("xvSSDTransPy::initialize: img_width=%d img_height=%d\n", img_width, img_height);

		xv_frame = new IMAGE_TYPE(img_width, img_height);
		xv_data = (char*) (xv_frame->data());

		printf("xv_frame: %d\t xv_data: %d\n", xv_frame, xv_data);
	...
	XVROI xv_roi(*(new_tracker->xv_size), init_pos);
	XVROI init_roi_min( *(new_tracker->xv_size), init_pos_min);
	IMAGE_TYPE init_template = subimage(*xv_frame, init_roi_min);

	ofstream fout("C++/log/init_template_py.txt", ios::out);
	fout<<init_template;
	fout.close();

	ofstream fout2("C++/log/xv_frame_py.txt", ios::out);
	fout2<<*xv_frame;
	fout2.close();
	//win_int=new WIN_INT(*xv_frame);
	//win_int->map();
	...
	//std::cin>>ch;
	printf("Initializing new tracker with pos_x: %d pos_y: %d\n", init_pos.PosX(), init_pos.PosY());
	new_tracker->ssd=new TRACKER_TYPE(STATE_TYPE(init_pos), init_template);
	//new_tracker->ssd=new TRACKER_TYPE;
	//printf("Setting stepper for the  new tracker\n");
	//new_tracker->ssd->setStepper(init_template);
	//printf("Initializing state for the  new tracker\n");
	//new_tracker->ssd->initState( (STATE_TYPE)xv_roi );
	printf("done !\n");
...
static PyObject* update(PyObject* self, PyObject* args) {
	...
	if (current_tracker->direct_capture) {
		...
		if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
			printf("\n----xvSSDTransPy::update: Input arguments could not be parsed----\n\n");
			return NULL;
		}
