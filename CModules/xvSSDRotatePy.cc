#include "xvSSDRotatePy.h"

/* ==== Set up the methods table ====================== */
static PyMethodDef xvSSDRotatePyMethods[] = {
...
PyMODINIT_FUNC initxvSSDRotatePy()  {
    (void) Py_InitModule("xvSSDRotatePy", xvSSDRotatePyMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "O!ddddiii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
		...
		&new_tracker->direct_capture, &new_tracker->show_xv_window)) {
			printf("\n----xvSSDRotatePy: initialize: Input arguments could not be parsed----\n\n");
			return NULL;
	}

	if (in_img_py == NULL) {
		printf("\n----xvSSDRotatePy::update: in_img_py is NULL----\n\n");
		return NULL;
	}

	printf("Initializing Rotate SSD Tracker with:\n\t");
	printf("size_x=%f\n\t",size_x);
	printf("size_y=%f\n\t",size_y);
	...
	if (new_tracker->direct_capture) {
		...
		if(vid) {
			...
			if (fp==NULL) {
				fprintf(stdout, "xvSSDRotatePy::initialize: File xvInput.bin could not be opened for reading\n");
				return NULL;
			}
		...
		if(!vid) {
			printf("xvSSDRotatePy::initialize: vid was not initialized successfully\n");
			return NULL;
		} else {
		...
		printf("xvSSDRotatePy::initialize: img_width=%d img_height=%d\n", img_width, img_height);
	...
	state.angle=0.0;
	new_tracker->ssd=new TRACKER_TYPE(state, init_template);
...
static PyObject* update(PyObject* self, PyObject* args) {
    ...
    if (current_tracker->direct_capture) {
        //printf("\n----xvSSDRotatePy: update: Using direct capture----\n\n");
        ...
        if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
            printf("\n----xvSSDRotatePy::update: Input arguments could not be parsed----\n\n");
            return NULL;
        }
