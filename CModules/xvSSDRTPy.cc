...
static PyMethodDef xvSSDRTPyMethods[] = {
...
PyMODINIT_FUNC initxvSSDRTPy()  {
    (void) Py_InitModule("xvSSDRTPy", xvSSDRTPyMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "O!ddddiii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
		...
		&new_tracker->direct_capture, &new_tracker->show_xv_window)) {
			printf("\n----xvSSDRTPy: initialize: Input arguments could not be parsed----\n\n");
			return NULL;
	}

	if (in_img_py == NULL) {
		printf("\n----xvSSDRTPy::update: in_img_py is NULL----\n\n");
		return NULL;
	}

	printf("Initializing RT SSD Tracker with:\n\t");
	printf("size_x=%f\n\t",size_x);
	printf("size_y=%f\n\t",size_y);
	...
	if (new_tracker->direct_capture) {
		...
		if(vid) {
			...
			if (fp==NULL) {
				fprintf(stdout, "xvSSDRTPy::initialize: File xvInput.bin could not be opened for reading\n");
				return NULL;
			}
		...
		if(!vid) {
			printf("xvSSDRTPy::initialize: vid was not initialized successfully\n");
			return NULL;
		} else {
		...
		printf("xvSSDRTPy::initialize: img_width=%d img_height=%d\n", img_width, img_height);
...
static PyObject* update(PyObject* self, PyObject* args) {
    ...
    if (current_tracker->direct_capture) {
        //printf("\n----xvSSDRTPy: update: Using direct capture----\n\n");
        ...
        if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
            printf("\n----xvSSDRTPy::update: Input arguments could not be parsed----\n\n");
            return NULL;
        }
