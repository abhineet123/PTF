#include "xvSSDPyramidRTPy.h"

/* ==== Set up the methods table ====================== */
static PyMethodDef xvSSDPyramidRTMethods[] = {
...
PyMODINIT_FUNC initxvSSDPyramidRTPy()  {
    (void) Py_InitModule("xvSSDPyramidRTPy", xvSSDPyramidRTMethods);
...
static PyObject* initialize(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "O!ddddiidii", &PyArray_Type, &in_img_py, &pos_x, &pos_y,
		...
		&new_tracker->direct_capture, &new_tracker->show_xv_window)) {
			printf("\n----xvSSDPyramidalRT::initialize: Input arguments could not be parsed----\n\n");
			return NULL;
	}

	if (in_img_py == NULL) {
		printf("\n----xvSSDPyramidalRT::update: in_img_py is NULL----\n\n");
		return NULL;
	}
	...
	if (new_tracker->direct_capture) {
		...
		if(vid) {
			...
			if (fp==NULL) {
				fprintf(stdout, "xvSSDPyramidalRT::initialize: File xvInput.bin could not be opened for reading\n");
				return NULL;
			}
		...
		if(!vid) {
			printf("xvSSDPyramidalRT::initialize: vid was not initialized successfully\n");
			return NULL;
		} else {
		...
		printf("xvSSDPyramidalRT::initialize: img_width=%d img_height=%d\n", img_width, img_height);
...
static PyObject* update(PyObject* self, PyObject* args) {
    ...
    if (current_tracker->direct_capture) {
        //printf("\n----xvSSDPyramidalRT: update: Using direct capture----\n\n");
        ...
        if (!PyArg_ParseTuple(args, "O!", &PyArray_Type, &in_img_py)) {
            printf("\n----xvSSDPyramidalRT::update: Input arguments could not be parsed----\n\n");
            return NULL;
        }
